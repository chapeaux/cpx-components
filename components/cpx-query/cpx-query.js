class CPXQuery1 extends HTMLElement {
    static get tag() {
        return 'cpx-query';
    }
    template;
    ...
    _setFilters(item) {
        ...
        if (add) {
            if (this.activeFilters.has(item.group)) {
                ...
                this.activeFilters.set(item.group, new Set([
                    item.key
                ]));
            }
        } else {
    ...
    search() {
        ...
        if (this.url && (this.activeFilters && this.activeFilters.size > 0 || this.term !== null && this.term !== '' && typeof this.term !== 'undefined') || this.auto) {
            ...
            this.dispatchEvent(new CustomEvent('query-complete', evt1));
        }
    }
    prepTemplate() {
    ...
    renderTemplate(data, ele) {
        ...
        if (eltmpl) {
            matches = [
                ...atob(eltmpl).matchAll(/\${([^{]+[^}])}/g)
            ];
            data.forEach((v, k)=>{
                if (Number.isInteger(k)) {
                    let html = matches.reduce((a, c)=>{
                        let dataVal = c[1].split('.');
                        return a.replaceAll(c[0], dataVal.length <= 1 ? v[c[1]] : dataVal.reduce((acc, curr)=>acc[curr]
                        , v));
                    }, atob(eltmpl));
                    ele.innerHTML += html;
                }
...
window.customElements.define(CPXQuery1.tag, CPXQuery1);
export { CPXQuery1 as CPXQuery };
