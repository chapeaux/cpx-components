class CPXWebSocket1 extends HTMLElement {
    template;
    _url = '';
    get url() {
        return this._url;
    ...
    set ready(val) {
        ...
        this.dispatchEvent(new CustomEvent('cpx-socket-ready', {
            bubbles: true,
            composed: true
        }));
    }
    ...
    logMessage(e) {
        ...
        const msgData = new Map(Object.entries(message));
        if (typeof message.length === 'number') {
            msgData.set('length', message.length);
        }
        this.data = msgData;
    }
    logError(e) {
...
window.customElements.define('cpx-websocket', CPXWebSocket1);
export { CPXWebSocket1 as CPXWebSocket };
