...
class CPXAuth1 extends HTMLElement {
    ...
    get kc() {
        return this.kcUrl.length > 0 && this.kcRealm.length > 0 && this.kcClientId.length > 0 || this.kcConfig.length > 0;
    }
    _kcAuto = false;
    get kcAuto() {
        return this._kcAuto;
    ...
    camelCase(str, to = true) {
        return to ? str.replaceAll(/-([a-z])/g, (m, g)=>g.toUpperCase()
        ) : str.replaceAll(/([a-z][A-Z])/g, (m, g)=>`${g[0]}-${g[1].toLowerCase()}`
        );
    }
    async kcInit(config) {
