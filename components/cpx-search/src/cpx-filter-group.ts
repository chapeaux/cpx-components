...
export class CPXFilterGroup extends HTMLElement {
    get html() {
        return `
        <style>
            :host {
    ...
    static get tag() { return 'cpx-filter-group'; }
    ...
    connectedCallback() {
        ...
        let slotItems = this.querySelectorAll('cpx-filter-item[slot]').length;
        if (slotItems === 0) {
...
window.customElements.define(CPXFilterGroup.tag, CPXFilterGroup);