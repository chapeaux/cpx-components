...
${this.thumbnail
            ? `<div class="thumb"><img src="${this.thumbnail.replace("http:", "https:")}"></div>`
            : ""}`;
    }
    get layout() {
        return this._layout;
    ...
    get url() {
        ...
        return !this.premium
            ? this._url
            : `https://broker${stage}.redhat.com/partner/drc/userMapping?redirect=${encodeURIComponent(this._url)}`;
    }
    set url(val) {
        if (this._url === val)
            return;
    ...
    computeURL(result) {
        if (result.fields && result.fields.sys_type === "book" &&
    ...
    computePremium(result) {
        var premium = false;
        if (result._type === "rht_knowledgebase_article" ||
