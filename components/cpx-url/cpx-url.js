export class CPXUrl extends HTMLElement {
  ...
  set filters(val) {
    ...
    this._filters.forEach((val, key) => {
      this.uri.searchParams.append(
        "f",
        `${key}~${Array.from(val).reduce((acc, curr) => acc + " " + curr)}`,
      );
    });
  }
  get sort() {
  ...
  set qty(val) {
    ...
    this._qty = val;
    this.setAttribute("qty", this._qty);
  }
  get init() {
    return this._init;
  }
  ...
  _setFilters(filtersQS) {
    return new Map(
      filtersQS.map(
        (o) => [o.split("~")[0], new Set(o.split("~")[1].split(" "))],
      ),
    );
  }
  _changeAttr(e) {
    switch (e.type) {
      ...
      case "search-complete":
        if (
          e.detail && typeof e.detail.term !== "undefined" &&
          e.detail.term.length > 0
        ) {
    ...
    if (e.detail && typeof e.detail.invalid === "undefined") {
      history.pushState(
        {},
        `RHD Search: ${this.term ? this.term : ""}`,
        `${this.uri.pathname}${this.uri.search}`,
      );
    } else {
      this.term = "";
      this.filters.clear();
